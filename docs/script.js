...
const SUPABASE_ANON_KEY = "sb_publishable_N2d3qVdYaUYU6ajEdSSTog_rAB0DKg";
...
async function initDetailPage() {
    ...
    if (!id) {
        document.getElementById('entry-detail').innerHTML = '<p class="error">No ID.</p>';
        return;
    }
    ...
    try {
        ...
        if (!entry) {
            document.getElementById('entry-detail').innerHTML = '<p class="error">Not found.</p>';
            return;
        }
...
async function loadCurrentEntries() {
    const body = document.querySelector('#current-table tbody');
...
async function loadOldEntries() {
    const body = document.querySelector('#old-table tbody');
